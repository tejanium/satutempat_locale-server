  ...
  describe '.import' do
    context 'one level create' do
      it 'create all keys and values' do
        ...
        }.should change { SatutempatLocale::Server::Storage.count }.from(0).to(3)
      end
      ...
      context 'datas' do
        ...
        it 'should have all keys' do
          SatutempatLocale::Server::Storage.all.to_a.map(&:key).sort
            .flatten.should eq ['one', 'two', 'three'].sort
        end

        it 'should have all values' do
          SatutempatLocale::Server::Storage.all.to_a.map(&:value).sort
            .flatten.should eq ['One', 'Two', 'Three'].sort
        end

        it 'should have all file_path' do
          SatutempatLocale::Server::Storage.all.each do |storage|
    ...
    context 'one level update' do
      ...
      it 'create all keys and values' do
        lambda{
          SatutempatLocale::Server::Storage.import 'spec/fixtures/one_level_updated.yml'
        }.should_not change { SatutempatLocale::Server::Storage.count }.from(3).to(6)
      end
    ...
    context 'one level added' do
      ...
      it 'create all keys and values' do
        lambda{
          SatutempatLocale::Server::Storage.import 'spec/fixtures/one_level_added.yml'
        }.should change { SatutempatLocale::Server::Storage.count }.from(3).to(4)
      end
